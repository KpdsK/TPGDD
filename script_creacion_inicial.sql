...
BEGIN
EXEC sp_executesql N'CREATE SCHEMA DESCONOCIDOS4 AUTHORIZATION [gd]' 
END
GO
/*---------Limpieza de Tablas-------------*/
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJE_RANGO_OK','FN') IS NOT NULL
...
BEGIN
	...
	DECLARE @CONT INT	
	SET @CONT=0
	-- QUE EL CLIENTE ESTE HABILITADO
	IF (SELECT Cliente_Habilitado FROM [DESCONOCIDOS4].CLIENTE WHERE Cliente_Id=@Clie )=1
	SET @CONT=@CONT+1
	-- QUE LA UNIDAD DISPONIBLE SEA UNICA Y HABILITADA
	IF (SELECT COUNT(*) FROM [DESCONOCIDOS4].UNIDAD_DISPONIBLE WHERE CONCAT(Uni_Dis_Chofer,Uni_Dis_Auto,Uni_Dis_Turno)=CONCAT(@Chof,@Auto,@Turno) AND Uni_Dis_Habilitado=1)=1
	SET @CONT=@CONT+1
	-- QUE LOS KILOMETROS MARCADOS SEAN MAYOR A 0
	IF @Cant_KM >0
	SET @CONT=@CONT+1
...
SET @FECHA_HASTA = (SELECT CONVERT(DATE,MAX(Viaje_Fecha_Hora_Fin)) FROM DESCONOCIDOS4.VIAJE WHERE Viaje_Cliente = @CLIENTE)

DECLARE @ESTADO_NUEVO INT
SET @ESTADO_NUEVO = (SELECT Cliente_Habilitado FROM INSERTED)	
	IF(@ESTADO_VIEJO = 1 AND @ESTADO_NUEVO=0 AND [DESCONOCIDOS4].FN_DATOS_A_FACTURAR(@CLIENTE, @FECHA_HASTA) =1)
...
--BUSCAR Y LISTAR AUTOS habilitados
...
GO
--COMPRUEBA QUE no hay datos a facturar
IF OBJECT_ID(N'[DESCONOCIDOS4].FN_DATOS_A_FACTURAR',N'FN')IS NOT NULL
	DROP FUNCTION [DESCONOCIDOS4].FN_DATOS_A_FACTURAR;
GO

CREATE FUNCTION [DESCONOCIDOS4].FN_DATOS_A_FACTURAR(@CLIENTE INT , @FECHA_HASTA DATE)
RETURNS BIT
AS
BEGIN
	DECLARE @RESUL BIT
...
AS 
BEGIN
IF ((SELECT Cliente_Habilitado FROM DESCONOCIDOS4.CLIENTE WHERE Cliente_Id = @CLIENTE) = 1 AND [DESCONOCIDOS4].FN_DATOS_A_FACTURAR(@CLIENTE, @FECHA_HASTA) =1)
	BEGIN
		SELECT 
...
BEGIN TRANSACTION
	  INSERT INTO [DESCONOCIDOS4].CABECERO_RENDICION(Cab_Rend_Nro,Cab_Rend_Turno,Cab_Rend_Chofer,Cab_Rend_Fecha,Cab_Rend_Importe) 
	   ...
	   AND CONVERT(DATE,Viaje_Fecha_Hora_Fin) <= CONVERT(DATE,@FECHA_RENDICION)
...
/*IF OBJECT_ID('GD1C2017.gd_esquema.Maestra') IS NOT NULL
	DROP TABLE GD1C2017.gd_esquema.Maestra;
GO
*/
